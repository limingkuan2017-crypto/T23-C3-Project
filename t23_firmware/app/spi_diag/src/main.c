...
/**
 * @brief Transfer one raw SPI buffer and print the returned bytes.
 *
 * @param tx Buffer to send.
...
static int spi_transfer_bytes(const uint8_t *tx, size_t len)
{
    ...
    if (tx == NULL || len == 0 || len > sizeof(rx)) {
        fprintf(stderr, "invalid transfer length: %zu\n", len);
        return 1;
    }
