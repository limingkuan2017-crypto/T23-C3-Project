...
/*
 * Channel table inherited from the vendor sample. The diagnostic entry point
 * only enables the subset needed for the active sensor config.
 */
...
/**
 * @brief Initialize the sensor, ISP and IMP system stack.
 *
 * This is the foundational bring-up function for the local video path. It
 * opens the ISP, registers the active sensor, enables the sensor, initializes
 * the IMP system and applies a small amount of ISP tuning.
 *
 * @param sensor_cfg Runtime sensor configuration selected by the diagnostic
 *                   entry point.
 *
...
int sample_system_init(sample_sensor_cfg_t sensor_cfg)
{
	...
	/*
	 * Bring up the minimal local video stack:
	 * 1. open ISP
	 * 2. add and enable sensor
	 * 3. initialize IMP system
	 * 4. enable tuning
	 *
	 * If this function fails, the problem is below framesource/JPEG and we
	 * should look at sensor, tx-isp, libimp or kernel compatibility first.
	 */
	...
	chn[CH0_INDEX].enable = sensor_cfg.chn0_en;
	chn[CH0_INDEX].fs_chn_attr.crop.width = sensor_cfg.sensor_width;
	chn[CH0_INDEX].fs_chn_attr.crop.height = sensor_cfg.sensor_height;
	chn[CH0_INDEX].fs_chn_attr.scaler.outwidth = sensor_cfg.sensor_width;
	chn[CH0_INDEX].fs_chn_attr.scaler.outheight = sensor_cfg.sensor_height;
	chn[CH0_INDEX].fs_chn_attr.picWidth = sensor_cfg.sensor_width;
	chn[CH0_INDEX].fs_chn_attr.picHeight = sensor_cfg.sensor_height;

/*
	chn[CH3_INDEX].fs_chn_attr.crop.width = sensor_cfg.sensor_width;
	chn[CH3_INDEX].fs_chn_attr.crop.height = sensor_cfg.sensor_height;
	chn[CH3_INDEX].fs_chn_attr.scaler.outwidth = sensor_cfg.sensor_width;
	chn[CH3_INDEX].fs_chn_attr.scaler.outheight = sensor_cfg.sensor_height;
	chn[CH3_INDEX].fs_chn_attr.picWidth = sensor_cfg.sensor_width;
	chn[CH3_INDEX].fs_chn_attr.picHeight = sensor_cfg.sensor_height;
*/
	memset(&sensor_info, 0, sizeof(IMPSensorInfo));
	memcpy(sensor_info.name, sensor_cfg.sensor_name, strlen(sensor_cfg.sensor_name));
	sensor_info.cbus_type = sensor_cfg.sensor_cubs_type;

	memcpy(sensor_info.i2c.type, sensor_cfg.sensor_name, strlen(sensor_cfg.sensor_name));
	sensor_info.i2c.addr = sensor_cfg.sensor_i2c_addr;
	sensor_info.i2c.i2c_adapter_id = sensor_cfg.sensor_i2c_adapter_id;
	...
	// 设置手动白平衡
	IMPISPWB imp_pwb = {
		.mode = ISP_CORE_WB_MODE_MANUAL,
		.rgain = 500,
		.bgain = 500
	};
	IMP_ISP_Tuning_SetWB(&imp_pwb);
...
/**
 * @brief Tear down the sensor, ISP and IMP system stack.
 *
 * @return 0 on success, -1 on failure.
 */
int sample_system_exit()
{
	...
	/* Tear down the ISP stack in reverse order to keep later runs clean. */
...
/**
 * @brief Enable all configured frame-source channels.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Disable all configured frame-source channels.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Worker thread that grabs raw frames from one frame-source channel.
 *
 * @param args Channel index encoded as a pointer-sized integer.
 *
 * @return `(void *)0` on success, `(void *)-1` on failure.
 */
static void *get_frame(void *args)
{
	...
	ret = IMP_FrameSource_SetFrameDepth(chnNum, chn[index].fs_chn_attr.nrVBs);
	if (ret < 0) {
	...
	for (i = 0; i < NR_FRAMES_TO_SAVE; i++) {
		...
		if (NR_FRAMES_TO_SAVE/2 == i) {
			if (write(fd, (void *)frame->virAddr, frame->size) != (int)frame->size) {
...
/**
 * @brief Start frame-grab worker threads for all enabled channels.
 *
 * @return 0 on success, -1 on failure.
 */
...
int sample_framesource_init()
{
	...
	/* Create logical frame-source channels on top of the working ISP path. */
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
...
/**
 * @brief Destroy all enabled frame-source channels.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Create and register JPEG encoder channels for enabled video channels.
 *
 * @return 0 on success, -1 on failure.
 */
int sample_jpeg_init()
{
	...
	/*
	 * Create JPEG encoder channels. This is the first layer that depends on the
	 * codec/VPU path, so failures here usually do not implicate basic sensor
	 * bring-up.
	 */
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (chn[i].enable) {
...
/**
 * @brief Create and register normal video encoder channels.
 *
 * This function is inherited from the vendor sample and is kept as a more
 * complete encoder initialization helper beyond the JPEG-only diagnostic path.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Unregister and destroy encoder channels created earlier.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Create a sample OSD group with multiple demonstration regions.
 *
 * This helper is inherited from the vendor sample. It is not part of the
 * current camera bring-up path, but it remains useful as reference code for
 * adding OSD overlays later.
 *
 * @param grpNum Encoder/OSD group number.
 *
 * @return Pointer to an array of OSD region handles on success, `NULL` on
 *         failure.
 */
IMPRgnHandle *sample_osd_init(int grpNum)
{
	...
	rAttrCover.data.coverData.color = OSD_IPU_RED; //Please use IMPIpuColour enum
	...
	rAttrRect.data.lineRectData.color = OSD_IPU_BLUE;//Please use IMPIpuColour enum
...
/**
 * @brief Stop and destroy OSD regions created by `sample_osd_init()`.
 *
 * @param prHander Region handle array returned by `sample_osd_init()`.
 * @param grpNum Encoder/OSD group number.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Write one IMP encoder stream into an already-open file.
 *
 * @param fd Output file descriptor.
 * @param stream Stream returned by IMP encoder APIs.
 *
 * @return 0 on success, -1 on failure.
 */
static int save_stream(int fd, IMPEncoderStream *stream)
{
	...
	for (i = 0; i < nr_pack; i++) {
		...
		if (ret != (int)stream->pack[i].length) {
...
/**
 * @brief Save one stream object into a file built from a prefix and index.
 *
 * @param stream_prefix File name prefix.
 * @param idx Sequential index appended to the file name.
 * @param stream Stream returned by IMP encoder APIs.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Worker thread that receives encoded video or JPEG data for one channel.
 *
 * @param args Packed argument containing payload type and channel number.
 *
 * @return `(void *)0` on success or an error-like pointer on failure.
 */
...
/**
 * @brief Start encoder worker threads for all enabled channels.
 *
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Capture a batch of JPEG snapshots and save them to disk.
 *
 * @return 0 on success, -1 on failure.
 */
int sample_get_jpeg_snap()
{
	...
	/* Save several JPEG frames so a human can quickly verify valid output. */
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			...
			for (j = 0; j < NR_JPEG_TO_SAVE; j++) {
				sprintf(snap_path, "%s/snap-%d-%d.jpg",
						SNAP_FILE_PATH_PREFIX, chn[i].index,j);
...
/**
 * @brief Receive encoder output using file descriptors and `select()`.
 *
 * Compared with `sample_get_video_stream()`, this version waits on encoder
 * file descriptors directly instead of using one thread per channel.
 *
 * @return 0 on success, -1 on failure.
 */
