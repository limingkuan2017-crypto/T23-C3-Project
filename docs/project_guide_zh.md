...
- 再在 rectified 图里选“内容完整优先、无黑边、比例自然”的最终显示区域
...
3. rectified 画布尺寸按当前 `quad` 自适应估算，不再强制压成 `16:9`
4. 最终 crop 先保留用户框出的电视内容，再尽量扩到无黑边的最大有效矩形
5. rectified 预览和后续边框取色共用同一套 rectification model 和 crop
6. crop 搜索已经同时解决：
   - 顶部黑块
   - 下身被过度裁掉
