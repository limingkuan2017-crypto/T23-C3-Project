...
#define T23_C3_PROTO_VERSION 0x0003u
...
typedef enum {
    ...
    T23_C3_FRAME_TYPE_RESP_MOSAIC_RGB = 0x52,
} t23_c3_frame_type_t;

#define T23_C3_PREVIEW_MOSAIC_WIDTH 16u
#define T23_C3_PREVIEW_MOSAIC_HEIGHT 16u
#define T23_C3_PREVIEW_MOSAIC_RGB_LEN \
    (T23_C3_PREVIEW_MOSAIC_WIDTH * T23_C3_PREVIEW_MOSAIC_HEIGHT * 3u)
...
#define T23_C3_RUN_MAGIC1 0x4Du /* 'M' */
#define T23_C3_RUN_STREAM_VERSION 0x01u

/*
 * Legacy dedicated runtime frame definition.
 *
 * The current code path mainly streams the runtime 16x16 mosaic with the same
 * fixed SPI frame container used elsewhere (`RESP_MOSAIC_RGB`), but this
 * structure is kept as a documented binary layout reference because earlier
 * bring-up versions and debug notes used it explicitly.
 */
typedef struct __attribute__((packed)) {
    uint8_t magic0;
    uint8_t magic1;
    ...
    uint8_t reserved1;
    uint16_t seq;
    t23_rgb8_t cells[T23_C3_PREVIEW_MOSAIC_WIDTH * T23_C3_PREVIEW_MOSAIC_HEIGHT];
    uint16_t checksum;
} t23_c3_run_mosaic_frame_t;
