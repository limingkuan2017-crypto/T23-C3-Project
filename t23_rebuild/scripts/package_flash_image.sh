...
cp "$VENDOR_REF/build/images/root-uclibc-toolchain540.squashfs" "$TMP_DIR/"
cp "$VENDOR_REF/build/images/root-uclibc-toolchain540.squashfs" "$TMP_DIR/root.img"
...
# 6. 预分配整片 flash 大小，保证最终输出和 16 MB flash 一致。
truncate -s "${SIZE_16M}" "$TMP_DIR/$IMAGE_NAME"

# 7. 按分区偏移把 bootloader / kernel / rootfs / system 依次写进整镜像。
...
rootfs     : root-uclibc-toolchain540.squashfs
EOF
